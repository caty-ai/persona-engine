...
- **場面に合わせて、表情と話し方が変わる。** 仕事の場では短く頼もしく、プライベートでは親しい友人や恋人のような距離の近さで、雑談では柔らかく — 同じエージェントが、場面ごとの顔を持てます。
- **人間と同じように、自然に切り替わる。** 切り替え方は 3 つ。①場所による自動切替（どこで話しているかをもとに、エンジンが毎回自動で顔を選ぶ）、②エージェント自身の判断（あなたが許可した場所でだけ、会話の流れを読んで自分で切り替える）、③あなたの一言（「集中モードにして」）。
- **出てはいけない場所では、必ずよそゆきの顔になる。** 会社のミーティングや仕事の場など、設定していない場面・知らない場面では、自動的に中立な状態に切り替わります。プライベートな口調が公開の場に漏れる事故を、しくみとして防ぎます。
- **元の人格は書き換えない。** persona-engine がやるのは、いまの場面に合った「表情のレイヤー」を会話のたびにそっと重ねることだけ。モードを外せば、エージェントはいつでも元の姿に戻ります。
- **いつ・どこで・誰がどの顔に切り替えたか、記録がぜんぶ残る。** 「昨日のあの会話、どのモードだったっけ？」に、いつでも答えられます。
- **表情も語彙も、あなた好みに育てられる。** 表情は「モード」、言葉づかいは「語彙テンプレート（カタログ）」という普通のテキストファイルでできていて、コピーして書き足すだけで増やせます。

## しくみはかんたん — モードと語彙テンプレート

自動で切り替わるしくみの中身は、たった 2 種類の部品です。表情の一つひとつは**モード**という小さな定義ファイル、言葉づかい・口ぐせ・応答の例は**語彙テンプレート（カタログ）**というテキストファイル。どちらも普通のテキストなので、書き足せばエージェントの表情と声が増え、書き換えれば好みに調整できます。「どの場所でどの表情を出してよいか」「誰が切り替えられるか」はルール（ルートポリシー）として別に決めるので、表情を増やしても安全は崩れません。

モードの追加方法と、語彙テンプレートの書き方・ルールは [カスタマイズガイド](docs/customizing.md) にまとめてあります。

## いちばん大切にしていること — 人格は書き換えない

persona-engine は、人格をゼロから作るツールではありません。**すでにそこにいるエージェントを尊重し、その上に感情のレイヤーを重ねる**ためのものです。

- エージェント本体の人格設定（名前・性格・デフォルトの言葉づかい）には、一切手を触れません。
