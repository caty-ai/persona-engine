...
## persona-engine 增加的——只有两样

### 关系的层次

即使是同一个智能体，关系在每个地方也略有不同。工作时是可靠的秘书；闲聊时是亲近的朋友；直播时是角色；公开场合则是得体的中立面孔。persona-engine 用一条明确的规则记住**哪张关系的面孔可以出现在哪里**，并在每轮对话上轻轻叠上恰好一层与场合相配的表情。

- **像人一样自然地切换。** 换脸有三种方式：① 按场所自动切换（引擎根据对话发生的地方，每一轮自动选择合适的面孔）；② 智能体自行判断（只在你允许的地方，它读懂对话的走向自己切换）；③ 你的一句话（"切换到朋友模式"）。
...
人的情感不是"工作/私下"的二选一，而是一条渐变。有好消息时声音亮起来，深度专注时话变短。在 persona-engine 里，渐变上的每一个点就是一个**模式**——一个小小的文本文件；声音的质感——偏好的说法、口头禅、示例回应——放在**词汇目录**里培养。情感变体可以从基准面孔继承，只携带自己的差异（`extends`）。这条渐变随着你们共同的生活慢慢变深：复制文件、编辑、重新构建——仅此而已。
...
> [!IMPORTANT]
> persona-engine 不是从零构建人格的工具。它的存在是为了**尊重已经在那里的智能体，在其之上叠加关系与情感**。

- 智能体自身的人格定义——名字、性格、默认的说话方式，无论放在 system prompt、`AGENTS.md` 还是 `SOUL.md`——从不被触碰。它始终是一切层次叠加的地基。
...
日语里有一个词，"言灵"（kotodama）——相信言语自有灵魂。一个文件一个文件地挑选智能体可以说出口的话，正是这样一件事：用语言，把魂注入你的智能体。而回应你的那句话，究竟是机械的输出，还是真正带着感情的言语——我们认为这不是定义能裁决的。人类的"心"在哪里，同样无人能定义，我们却依然相信它。这件事，希望你在关系逐渐建立的过程中亲自体会。persona-engine 就是为帮助这件事发生而造的装置。
...
构成情感渐变的零件有两种：每张面孔是一个**模式**——一个小小的定义文件；用词、口头禅和示例回应放在**词汇目录**里——模式引用的普通文本文件。构成关系层次的规则只有一条：**路由策略**，决定哪张面孔可以出现在哪里、谁可以切换。仅此而已。添加文件，智能体的表情就增多；因为规则单独存放，增加表情不会削弱安全。
...
**已验证环境:** 核心套件会在每个拉取请求中分别于 Ubuntu 和 macOS 上运行（`make test`、Node 22+）；适配器套件（Python 和 Node 工作区）在 Ubuntu 上运行。WSL2 需要将检出放在 Linux 文件系统（`~/…`）中；在 Windows 挂载盘（`/mnt/c`，DrvFs）上，测试套件会对权限/FIFO/符号链接用例给出明确原因并跳过，而不是失败。
...
| 纵轴 | **Persona Engine** | 为智能体赋予人格 — 分层人格与情感渐变 | 已公开・MIT |
